<?php
    ...
    $count = "";
        ...
        if (isset($_POST["insert"])) {
            ...
            $sql = "SELECT  SUM(ENROLLED) from class_size";
            $result = $con->query($sql);
            $count = mysqli_num_rows($result);
            while($row = mysqli_fetch_array($result)){
                    $totalSecInUni=$row['SUM(ENROLLED)'];
            }
            ...
            for($i=0;$i<Count($schools);$i++){
                ...
                $allData ="SELECT ENROLLED , ROOM_CAPACITY FROM class_size WHERE SCHOOL_TITLE='$school' AND Semester='$semester' AND Year='$year'";
